    ...
    for j = 1:num_sat
            ...
            if(startIndex > 0)
                if(line(1) == 'C') %if(line(1) == 'G')
                    ...
                    elseif((line(6:19)) == '              ') 
                       %continue;
                       epoch_data(j,3:6) = 0.0000000;
                       if(83 > (numel(line))) 
                             epoch_data(j,:) = 0.0000000;    %%Changed this, make edit to other obs files        
                       elseif((line(70:83)) == '              ')  
                            epoch_data(j,:) = 0.0000000;
                            epoch_data(j,1) = 0.0000000;
                            epoch_data(j,2) = 0.0000000;
                       else
                            line(70:83) = spacereplace(line(70:83)); 
                            epoch_data(j,7) = str2num(line(70:83)); 
                            epoch_data(j,8:10) = 0.0000000;
                       end
                       line = fgetl(file);
                       continue;
                    else
                    ...
                    line = fgetl(file);
%                     
%                     if(epoch_data(j,3) == 0.0000000)
%                          if(epoch_data(j,7) == 0.0000000)
%                              epoch_data(j,:) = []; 
%                          end
%                     end
...
%                 line = fgetl(file);
    ...
    ind = find(sum(epoch_data,2)==0) ; %%should be 2
    epoch_data(ind,:) = [];
