...
%% Least squares portion
    ...
    xcurrentt = xapproxt;   ycurrentt = yapproxt;   zcurrentt = zapproxt;   dtrcurrentt = 0;
    countt = 0;   
    tabt = 0;
    ...
    while(countt < 40)
        clear PRt At mist
        x0t = xcurrentt;   y0t = ycurrentt;   z0t = zcurrentt;  dtr0t = dtrcurrentt; %Setting the current estimates of the unknown parameters to the current least squares estimates of the parameters
    
    for j = 1:obssize(1) %sizeofobs(1) %A for-loop to create the first design matrix
        %Determining the geometric range
        %x0 = xcurrent, y0 = ycurrent, z0 = zcurrent
        geometricexpressiont = (Xst(j) - xcurrentt)^2 + (Yst(j)- ycurrentt)^2 + (Zst(j) - zcurrentt)^2;
        ...
        xot = [xcurrentt; ycurrentt; zcurrentt; dtr0t];  %[x0; y0; z0; dtr0]  %The current approximations of the receiver position and clock offset
...
for p = epochs:-1:1
    obsvalue = size(Observations{i,2});
    for k = length(obsvalue):-1:1
...
for i = 1:length(Observations) %Iterating through the number of observations
    newobssize = size(Observations{i,2}); %Determining the size of the submatrix attached to the epoch of observations
    clear elevationangle azimuth dTsv
     remove = [];
     removeobs = []; 
     removej = [];
     ...
     for j = 1:newobssize(1) %Iterating through the submatrix attached to the observation epoch
         %% Least squares portion
    ...
    for j = 1:newobssize(1) %sizeofobs(1) %A for-loop to create the first design matrix
        %Determining the geometric range
        %x0 = xcurrent, y0 = ycurrent, z0 = zcurrent
