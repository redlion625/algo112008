function [Nav,navGLO] = Nav_readergal(Navemerisfile);
...
fidobs = fopen('brdm1190.21p'); %brdm1190.21p   % open observation file
...
while line~=-1      % do until end of file is reach where fgets returns -1
    ...
    if(line(1) == 'G')
        data(i_epoch,1) = 1;
        data(i_epoch,2) = str2num(line(2:3)); %str2num(line(1:3));
        %elseif(line(1) == '
        ...
        data(i_epoch,3) = toGPST(year+2000,month,day,hour,minute,second);
        %data(i_epoch,2) = toGPST(year+2000,month,day,hour,minute,second);
        data(i_epoch,4) = str2num(line(24:42)); %SV Clock Bias
        data(i_epoch,5) = str2num(line(43:61)); %SV Clock Drift
        data(i_epoch,6) = str2num(line(63:end)); %SV Clock Drift Rate
        ...
        for i = (6+1):4:(27+1)
            ...
            %ind = end(A,k,n)
            emptytest = []; %'[]'
            if(length(line) > 61)    %isnumeric(str2num(line(62))) == 1)  %str2num(line(end)) == 0 && 
                data(i_epoch,i+3) = str2num(line(62:end));   %data(i_epoch,i+3) = 0;   %elseif((line(6:19)) == '              ')
            else
                data(i_epoch,i+3) = 0; %data(i_epoch,i+3) = str2num(line(62:end));
            end
        end
%         if(data(i_epoch,1) == 0)
%             data(i_epoch,:) = [];
%         end
        line = fgetl(fidobs);
        data(i_epoch,31) = str2num(line(5:23)); %Transmission Time
        if length(strfind(line,'.')) ~=1 %check if Fit Interval provided
            data(i_epoch,32) = str2num(line(25:42)); %Fit Interval
        end
        line = fgets(fidobs);
    elseif(line(1) == 'E')
        dataE(i_epoch,1) = 3;
        dataE(i_epoch,2) = str2num(line(2:3)); %str2num(line(1:3));
        %elseif(line(1) == '
        %data(i_epoch,1) = (line(1:3)); %PRN
        %data(i_epoch,1) = str2num(line(1:3)); %PRN
        year = str2num(line(5:8)) - 2000; %Epoch Year
        ...
        % convert the Epoch Year,Month, Day, Hour, Minute and Second to GPST
        dataE(i_epoch,3) = toGPST(year+2000,month,day,hour,minute,second);
        %data(i_epoch,2) = toGPST(year+2000,month,day,hour,minute,second);
        dataE(i_epoch,4) = str2num(line(24:42)); %SV Clock Bias
        dataE(i_epoch,5) = str2num(line(43:61)); %SV Clock Drift
        dataE(i_epoch,6) = str2num(line(63:end)); %SV Clock Drift Rate
        % remaining lines of data
        for i = (6+1):4:(27+1)
            line = fgetl(fidobs);
            dataE(i_epoch,i) = str2num(line(5:23));
            dataE(i_epoch,i+1) = str2num(line(24:42));
            dataE(i_epoch,i+2) = str2num(line(43:61));
            %ind = end(A,k,n)
            emptytest = []; %'[]'
            if(length(line) > 61)    %isnumeric(str2num(line(62))) == 1)  %str2num(line(end)) == 0 && 
                dataE(i_epoch,i+3) = str2num(line(62:end));   %data(i_epoch,i+3) = 0;   %elseif((line(6:19)) == '              ')
            else
                dataE(i_epoch,i+3) = 0; %data(i_epoch,i+3) = str2num(line(62:end));
            end
        end
%         if(data(i_epoch,1) == 0)
%             data(i_epoch,:) = [];
%         end
        line = fgetl(fidobs);
        dataE(i_epoch,31) = str2num(line(5:23)); %Transmission Time
        if length(strfind(line,'.')) ~=1 %check if Fit Interval provided
            dataE(i_epoch,32) = str2num(line(25:42)); %Fit Interval
        end
        line = fgets(fidobs);
...
%         line=fgets(fidobs);
    %end
    elseif(line(1) ~= 'G')  %if(line(1) ~= 'G'&&line(1)~='E')
        if(line(1) ~= 'E')
            %continue %break
            line = fgets(fidobs);
        end        
...
index_nonzerolines = sum(abs(data),2)>0 ;
% Create matrix B containing only the non-zero lines of A
corrdata = data(index_nonzerolines,:) ;

galindex_nonzerolines = sum(abs(dataE),2)>0 ;
corrdataE = dataE(galindex_nonzerolines,:) ;
