...
while ~feof(file)
    ...
    sat_names = strsplit(sat_names,'E'); %G
    ...
    for j = 1:num_sat
            ...
            startIndexE = regexp(strE,expression);
            if(all(startIndex) > 0 || all(startIndexE) > 0)  %if(startIndex > 0 || startIndexE > 0)
