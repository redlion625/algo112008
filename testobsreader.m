...
file = fopen("0105Mi8_GnssLog.21o"); %("Pixel4XLModded_GnssLog.20o");  %fopen("ALGO0010.08O");
...
%    sat_PRN(v) = str2num([sat_names{v}]); 
%     for j = 1:num_sat
%         sat_PRN(j) = str2num([sat_names{j}]);
%     end
    ...
    line = curr_line; 
    for j = 1:num_sat
        % iterate through each observation type in the epoch
%         if (str2num(line(2:3)) == "")
...
%         for k = 1:obs.num_typeobs
%             if k == 6
%                 line = fillWhite(fgetl(file));
%                 epoch_data(j,k+1) = str2double(line(1+16*(k-6) : 14+16*(k-6)));
%             elseif k > 6
%                 epoch_data(j,k+1) = str2double(line(1+16*(k-6) : 14+16*(k-6)));
%             else
%                 line = fillWhite(line);
%                 epoch_data(j,k+1) = str2double(line(1+16*(k-1) : 14+16*(k-1)));
%             end       
%         end
          line = fgetl(file);
