...
fidobs = fopen('brdm1180.21p'); %brdm1190.21p   % open observation file
...
while line~=-1      % do until end of file is reach where fgets returns -1
    ...
    if(line(1) == 'C')
        ...
        data(i_epoch,2) = toBDS(year+2000,month,day,hour,minute,second);
...
%         end
    ...
    elseif(line(1) ~= 'C')  %if(line(1) ~= 'G'&&line(1)~='E')
        %continue %break
         line = fgets(fidobs);
